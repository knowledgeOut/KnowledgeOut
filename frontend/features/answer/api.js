 ...
 * 답변 수정
 * @param {string|number} questionId - 질문 ID
 * @param {Object} data - { answerId, content }
 * @returns {Promise<Object>} 수정된 답변 정보
 */
export async function updateAnswer(questionId, data) {
  try {
    const response = await apiClient.put(`/questions/${questionId}/answers`, data);
    return response;
  } catch (error) {
...
export async function deleteAnswer(questionId, answerId) {
  try {
    const response = await apiClient.delete(`/questions/${questionId}/answers?answerId=${answerId}`);
    return response;
  } catch (error) {
